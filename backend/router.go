...
func setupRouter(h *Handler, cfg Config) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		api.GET("/projects", h.ListStudioProjects)
		api.POST("/projects", h.CreateStudioProject)
		api.GET("/projects/:id", h.GetStudioProject)
		api.PUT("/projects/:id", h.UpdateStudioProject)
		api.DELETE("/projects/:id", h.ArchiveStudioProject)
		api.PUT("/projects/:id/product-pack", h.PutProductPack)
		api.GET("/recipes", h.ListRecipes)
		api.GET("/generation-providers", h.ListGenerationProviders)
		api.POST("/projects/:id/recipe-runs", h.CreateRecipeRun)
