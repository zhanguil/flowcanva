...
function startEditRole(i: number) {
  editingRoleIndex.value = i
  Object.assign(editRole, roles.value[i])
}
...
<template>
  <div class="p-8 max-w-3xl mx-auto space-y-8">
    <!-- 标题头 -->
    <div>
      <h1 class="text-3xl font-extrabold tracking-tight text-base-content/90 mb-1">文本节点配置</h1>
      <p class="text-sm text-base-content/40 font-medium">配置 AI 模型、默认角色模板提示词和相关参数</p>
    </div>
    ...
    <div class="flex flex-col gap-6">
      ...
      <div class="card bg-base-100 border border-base-200/80 shadow-sm p-5 rounded-2xl">
        <label class="label cursor-pointer justify-between w-full">
          <div class="space-y-0.5">
            <span class="label-text font-bold text-base-content/80 text-sm">启用文本节点</span>
            <p class="text-[11px] text-base-content/40 font-medium">控制画布端文本生成模块是否激活并允许被选用</p>
          </div>
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <label class="form-control w-full">
              ...
              <div class="flex justify-between text-[10px] text-base-content/30 mt-1 font-medium"><span>0.0 (精细)</span><span>2.0 (创意)</span></div>
            </label>

            <label class="form-control w-full">
              <div class="flex items-center justify-between mb-1.5">
                <span class="text-xs font-bold text-base-content/50">Top P (核心采样)</span>
          ...
          <label class="form-control w-full">
            <div class="label py-1"><span class="label-text text-xs font-bold text-base-content/50">Max Tokens 单次文本响应的最大 Token 上限限制</span></div>
      ...
      <div class="card bg-base-100 border border-base-200/80 shadow-sm rounded-2xl overflow-hidden">
        <div class="border-b border-base-200/80 bg-base-200/25 px-6 py-4 flex items-center justify-between">
          <div class="space-y-0.5">
            <h3 class="text-sm font-bold text-base-content/80">预设角色模板库</h3>
            <p class="text-[10px] text-base-content/40 font-medium">定义各种专属角色（Persona），支持配置系统 System Prompt</p>
          </div>
        ...
        <div class="p-6">
          ...
          <div v-if="showAddRole" class="bg-base-200/10 rounded-2xl p-5 border border-base-200/80 space-y-4 shadow-inner">
            <div class="flex items-center justify-between border-b border-base-200/60 pb-2 mb-2">
              <h4 class="text-xs font-extrabold text-base-content/80">新增系统角色预设</h4>
            ...
            <div class="grid grid-cols-2 gap-3">
              <label class="form-control w-full">
                ...
                <input v-model="newRole.key" type="text" placeholder="translator" class="input input-bordered input-sm w-full focus:ring-1 focus:ring-primary/10 text-xs font-mono" />
              </label>
              <label class="form-control w-full">
                <div class="label py-0.5"><span class="label-text text-[10px] font-bold text-base-content/50">角色展示名称</span></div>
                <input v-model="newRole.label" type="text" placeholder="翻译助手" class="input input-bordered input-sm w-full focus:ring-1 focus:ring-primary/10 text-xs" />
              </label>
            </div>
            <label class="form-control w-full">
      ...
      <div class="card bg-base-100 border border-base-200/80 shadow-sm rounded-2xl overflow-hidden">
        <div class="border-b border-base-200/80 bg-base-200/25 px-6 py-4">
          <h3 class="text-sm font-bold text-base-content/80">默认提示词系统模板</h3>
        </div>
        <div class="p-6">
          <label class="form-control w-full">
            <div class="label py-0.5"><span class="label-text-alt text-[10px] font-bold text-base-content/30 font-semibold text-primary">支持用双花括号接收上游变量（如： &#123;&#123;content&#125;&#125;）</span></div>
            <textarea v-model="form.prompt_template" class="textarea textarea-bordered w-full h-32 focus:ring-2 focus:ring-primary/15 text-sm leading-relaxed p-4" placeholder="请根据以下内容生成摘要：&#10;&#10;{{content}}" />
          </label>
        </div>
      </div>
